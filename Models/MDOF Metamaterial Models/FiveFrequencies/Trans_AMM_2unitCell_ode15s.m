...
freq_range=[38.64]; % range from 10 Hz to 43 Hz     f1 f2 f3 f4 f5
...
stiff3=[1600]*stiff2; %100 200 400 800 1600
%% Solve the model
...
i=1;
        ...
        [t1,result1]=ode15s(@(t,z) rhs(t,z,omega,k3),t,z,options);
        
        % change result to show the steady state portion of the time history
        x=result1;%(p:q,:); % x becomes the steady state result
