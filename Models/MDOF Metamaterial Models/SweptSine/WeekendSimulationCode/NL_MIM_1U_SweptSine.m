...
z=zeros(1,1*4); % n=2 and there are 4 DOF per unit cell, hence 4 initial conditions;
...
%% Non-dimensionalised Results
    ...
    title('Transmittance curve for 2U system MIM model')
    grid on
...
%% Save the results
save 1UweaklyNLamp10.mat amplitude swept_sine_range t t_new
savefig('1UweaklyNLamp10.fig')
...
dxdt_2 = -((2*damp1+damp2)/mass1)*du1- ((2*stiff1)/mass1)*u1-(stiff2/mass1)*(u1-v1) -...
    (stiff3/mass1)*(u1-v1)^3+(damp2/mass1)*dv1 + ...
