%% Plots the transient response of a forced 5 unit cell MDOF metamaterial model
clear all
tic
...
z=zeros(1,5*4); % n=7 and there are 4 DOF per unit cell, hence 4 initial conditions;
...
function dxdt=rhs(t,x,omega)
        ...
        % Cell 5 (last mass)
         % first mass
        dxdt_9 = x(18);
        dxdt_10 = -((2*damp1+damp2)/mass1)*x(18)- ((2*stiff1+stiff2)/mass1)*x(17)...
